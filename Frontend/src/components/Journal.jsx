...
export default function MoodJournal() {
  ...
  const handleSubmit = async () => {
    ...
    if (note.length >= 5 && date && selectedMood !== null) {
      ...
      try{
        const response = await fetch('https://priv-safespace-6qcp.onrender.com/api/journal', {
          method: 'POST',
          headers: {
  ...
  if (entries.length > 0) {
    ...
    useEffect(() => {
      ...
      const fetchUserData = async () => {
        try {
          ...
          const response = await axios.get('https://priv-safespace-6qcp.onrender.com/user', {
            headers: {
    ...
    useEffect(() => {
      ...
      const fetchEntries = async () => {
        try {
          const response = await fetch(`https://priv-safespace-6qcp.onrender.com/api/journal/${userId}`);
          const data = await response.json();
          if (response.ok) {
            setEntries(data); 
          } else {
      ...
      if(entries.length>15 && mostCommonMood =='terrble'){
        const sendNotif = async ()=>{
          try {
            ...
            await axios.post("https://priv-safespace-6qcp.onrender.com/notify-admin", {
              message: '',
              userId: userId,
              timestamp: timestamp,
            });
